...
- **"Papá de" (rival más ganado)**: en cada tarjeta, si la persona le ganó alguna vez a alguien, se muestra una fila destacada "👑 Fulano papá de Mengano · N veces" con el rival al que más veces le ganó (calculado por separado para truco y podrida). En truco cuenta como "le ganó a" cada persona del equipo rival cuando gana el chico (si juegan de a varios por equipo, se cuenta cada cruce); en podrida, el que gana la partida "le gana" a todos los demás jugadores de esa partida. Si nunca ganó nada, no se muestra la fila. Lógica en `historial.js` (`sumarVictoriaContra`/`rivalMasVeces`, integrado en `calcularStatsPersonas()`), verificado con datos de prueba antes de tocar el grupo real.
