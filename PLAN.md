...
- **Dureza contra trampas de amigos técnicos**: como el código de grupo (y la config de Firebase) están en el JS público, cualquiera con conocimientos podría intentar llamar al SDK de Firestore directo desde la consola del navegador, saltándose la interfaz. Por eso la protección real no puede depender solo de "no mostrar el botón" — tiene que estar en las **reglas de seguridad de Firestore** (Console → Firestore → Reglas), que son la única barrera que Firestore respeta pase lo que pase en el cliente:
  - `historial`: permite `create`, pero **niega `delete`** y solo permite un `update` muy acotado (agregar `fotoUrl` una única vez, sin poder tocar ningún otro campo ni reemplazar una foto ya puesta — ver "Foto opcional" más abajo). Exige además que `fecha` sea literalmente el `request.time` del servidor (bloquea fechas inventadas).
...
- **El historial de partidas contadas es inmutable**: no existe ningún botón para borrar ni corregir una partida ya guardada (a propósito no hay `borrarHistorialNube()` ni nada parecido) — pedido explícito de Facundo para que el registro sea confiable y nadie pueda "borrar" una derrota. Como las partidas amistosas ni siquiera se guardan (ver "Partido amistoso" más abajo), todo lo que aparece en el historial del grupo es, por definición, una partida real. La restricción está reforzada en dos capas: la interfaz no ofrece la opción, **y** las reglas de Firestore niegan `delete` y casi todo `update` sobre `historial` a nivel de base de datos (ver "Dureza contra trampas de amigos técnicos" más arriba) — así que ni editando el código ni llamando al SDK a mano se puede sortear.
- **El guardado es instantáneo, no depende de ninguna animación**: cuando un chico de truco llega al objetivo, o termina la última ronda de una podrida, el resultado se guarda YA (`guardarResultadoChico()` / `guardarResultadoPodrida()`), antes de mostrar el festejo (10s en truco) o la animación de tabla (~1.2s en podrida). Antes esto no era así: la llamada a `agregarAlHistorial()` estaba metida adentro de la función que recién se ejecutaba al FINAL del festejo, así que si alguien cerraba la app en esos segundos (por ejemplo, justo porque perdió), la partida no quedaba guardada — pedido explícito de Facundo, detectado como bug real al pensar la función de la foto. `agregarAlHistorialNube()` en `nube.js` ahora es sincrónica (no `async`): genera el id del documento nuevo con `doc(...)` al toque (sin red de por medio) y dispara el `setDoc(...)` sin esperarlo — el SDK offline-first se encarga de encolarlo solo. Verificado con Playwright: a los 300-400ms de terminar la partida (mucho antes de que la animación/festejo termine) el historial ya tiene la entrada nueva.
- **Foto opcional al terminar una partida** (Firebase Storage): en el cartel de ganador (truco) y en el podio (podrida) aparece un botón "📷 Sacar foto de la pizarra (opcional)" — nunca un requisito, y nunca bloquea ni demora el guardado del resultado (que ya pasó antes, ver punto de arriba). Al elegir una foto (`<input type="file" accept="image/*" capture="environment">`, abre la cámara directo en el celular) se sube a Cloud Storage en `grupos/{codigo}/historial/{entradaId}.jpg` y se asocia a la entrada ya guardada con un `updateDoc` que las reglas solo dejan pasar si el ÚNICO campo tocado es `fotoUrl` y todavía no existía uno (no se puede reemplazar una foto ya puesta). Si falla (sin señal, Storage no configurado, etc.) se avisa con un mensaje claro en vez de quedarse colgado — `bucket.maxUploadRetryTime` se bajó a 10s (el default del SDK es reintentar hasta 2 minutos). El helper genérico `ui.prepararFotoOpcional(prefijo, entradaId, subirFn)` en `ui.js` maneja el wiring (botón/input/estado) y lo comparten truco y podrida. La foto, si existe, se ve como link "📷 foto" en la lista del Historial.
  - **Requiere Firebase Storage habilitado y con reglas propias** (Console → Storage), configuración manual pendiente — sin eso, el botón de foto funciona pero siempre falla al subir (probado: cae en el mensaje de error a los 10s, sin romper nada más).
