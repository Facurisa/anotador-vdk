...
- **El guardado es instantáneo, no depende de ninguna animación**: cuando un chico de truco llega al objetivo, o termina la última ronda de una podrida, el resultado se guarda YA (`guardarResultadoChico()` / `guardarResultadoPodrida()`), antes de mostrar el festejo (10s en truco) o la animación de tabla (~1.2s en podrida). Antes esto no era así: la llamada a `agregarAlHistorial()` estaba metida adentro de la función que recién se ejecutaba al FINAL del festejo, así que si alguien cerraba la app en esos segundos (por ejemplo, justo porque perdió), la partida no quedaba guardada — pedido explícito de Facundo, detectado como bug real al pensar la función de la foto (ver abajo). `agregarAlHistorialNube()` en `nube.js` es sincrónica (no `async`): genera el id del documento nuevo con `doc(...)` al toque (sin red de por medio) y dispara el `setDoc(...)` sin esperarlo — el SDK offline-first se encarga de encolarlo solo. Verificado con Playwright: a los 300-400ms de terminar la partida (mucho antes de que la animación/festejo termine) el historial ya tiene la entrada nueva.
- **Foto opcional al terminar una partida — descartada**: se llegó a implementar (Firebase Storage, botón "Sacar foto", regla de Firestore para asociarla) pero Facundo la descartó al enterarse de que Firebase Storage ahora exige pasar el proyecto al plan Blaze (pago por uso, tarjeta cargada) incluso para este uso mínimo — prefirió no tocar la facturación. Se revirtió todo el código de la foto (no quedó nada a medias en la UI); lo único que sobrevive de ese trabajo es el fix del guardado instantáneo (punto de arriba), que sigue siendo válido y necesario aparte de la foto. Si en algún momento Facundo cambia de opinión sobre el plan Blaze, se puede retomar desde el historial de git.
...
- **Salir del grupo**: botón "salir" junto al código en la pantalla Personas (`js/grupo.js` → `salirGrupo()`). Pide confirmación, corta los listeners de Firestore y vacía la caché en memoria (`nube.detenerListeners()`), borra `codigoGrupo` de `localStorage` y vuelve a la pantalla de grupo — desde ahí se puede crear otro grupo o unirse a uno distinto (por ejemplo, para pasarse del grupo que armó por error un amigo al grupo real del resto). No borra nada del lado del servidor: el grupo viejo sigue existiendo para quien lo use.
...
- **"Papá de" (rival más ganado)**: en cada tarjeta, si la persona le ganó alguna vez a alguien, se muestra una fila con el rival al que más veces le ganó (por separado para truco y podrida). Si le ganó a ese rival **más veces de las que perdió contra él** (el empate no alcanza), la fila es destacada: "👑 Fulano papá de Mengano · N veces" (si hay varios así, el de más victorias). Si no tiene ventaja sobre nadie, igual se muestra a quien más le ganó, pero en fila común ("Más le ganó a Mengano · N veces"), aunque sean pocas. Ej.: Facu le ganó 8 a Hugo y Hugo 5 a Facu → a Facu le sale "papá de Hugo"; a Hugo le sale "Más le ganó a Facu · 5 veces" (salvo que sea papá de otro). En truco cuenta como "le ganó a" cada persona del equipo rival cuando gana el chico; en podrida, el que gana la partida "le gana" a todos los demás de esa partida. Lógica en `historial.js` (`sumarVictoriaContra`/`rivalMasVeces`, integrado en `calcularStatsPersonas()`).
...
- **Foto de Pats al perder**: si en el equipo que pierde el chico hay una persona cuyo nombre contiene "pats" (sin importar mayúsculas ni emojis), aparece su foto (`img/pats.jpg`, achicada a 720px, cacheada por el service worker) a pantalla completa por encima del festejo y del cartel de ganador, durante 30 s (`DURACION_FOTO_PATS` en `js/truco.js`) o hasta que se toque. Es solo visual (no toca el guardado) y también aparece en partidos amistosos. Overlay `#overlay-pats` en `index.html`.
- **Versión visible**: en la pantalla de inicio, abajo, se ve en chico la versión del service worker (ej. "anotador-v28", leída de `caches.keys()` en `mostrarVersion()` de `app.js`) para confirmar qué versión corre en cada celular. La foto de Pats ignora toques el primer 0,8 s (`pointer-events:none`) para que el click que genera el navegador tras el último toque de puntaje no la cierre al instante. Si `guardarResultadoChico()` falla, se atrapa el error y el festejo/foto siguen igual (con un aviso).
...
- Agregar jugadores: nombre + color (paleta de ~8 tizas de colores, sin repetir). Se pueden agregar, renombrar o quitar jugadores también con la partida empezada (si se quita uno, conservar su historial pero sacarlo de rondas futuras). **Al agregar uno a mitad de partida** (menú → "Agregar jugador"), arranca empatado con quien va último en ese momento (no en 0), para no ponerlo en desventaja por las rondas que ya se jugaron sin él — se guarda como un "ajuste" de puntaje aparte (`estado.ajustes`), sin tocar el historial de rondas jugadas. Ese mismo agregado a mitad de partida también ofrece, debajo del campo de texto, fichas para elegir directo a alguien ya guardado del grupo (`renderGuardadosMenu()` en `podrida.js`) en vez de tener que volver a escribir el nombre — antes solo existía esa opción en la pantalla de configuración inicial (`crearSelectorPersonas` con `guardadosWrap`/`guardadosLista`), no acá.
