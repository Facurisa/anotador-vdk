...
- **El historial de partidas contadas es inmutable desde la app**: no existe ningún botón para borrar ni corregir una partida ya guardada (a propósito no hay `borrarHistorialNube()` ni nada parecido) — pedido explícito de Facundo para que el registro sea confiable y nadie pueda "borrar" una derrota. Como las partidas amistosas ni siquiera se guardan (ver "Partido amistoso" más abajo), todo lo que aparece en el historial del grupo es, por definición, una partida real. Esto es una restricción de la interfaz (nadie del grupo ve un botón para hacerlo), no una regla de seguridad a nivel de Firestore.
